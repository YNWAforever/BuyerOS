...
| Scope extension 2 (owner-approved, execution session) | `tests/conftest.py` was added to the modify set for Task 5. Unskipping the route-level isolation test exposed that `psycopg`'s async driver cannot run on Windows' default `ProactorEventLoop`, so the test fails before any SQL. The same 6-line win32 selector-loop pin already exists in `services/worker/tests/conftest.py`, which describes itself as mirroring the API conftest. Test-only; inert on non-Windows. |
