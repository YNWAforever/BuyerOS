...
async def validation_error_handler(request: Request, exc: Exception) -> JSONResponse:
    """Every non-2xx uses the contract envelope, not FastAPI's default `detail` shape."""
    return JSONResponse(
        status_code=422,
...
from .errors import ApiError, error_handler, http_error_handler, validation_error_handler
...
def test_route_tenant_scope_comes_from_the_membership_checked_path(seeded, monkeypatch):
    """Route-level confinement: a member of A must never read B's rows through the API."""
...
**Review-round-1 additions:** `pytest` import; the no-context case asserts the RLS policy
*raises* (`UndefinedObject`) rather than returning rows, which is the actual fail-closed
behavior; a route-level confinement test that is **skipped with the recorded blocker** rather
than deleted, so the constraint is tracked and the test can be unskipped with the BO-004
`users` grant. **NOT RUN** until then.
