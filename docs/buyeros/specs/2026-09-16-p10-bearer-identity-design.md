...
- **The verifier performs signature and algorithm checks only.** The JWT library's own claim verification (`aud`, `iss`, `exp`, `nbf`, `iat`) is disabled explicitly, because otherwise it would run *before* `claims_to_principal` and silently own `exp`/`nbf` — rejecting a token inside the 60s `nbf` skew that this design permits, and splitting claim ownership across two places. Disabling `iat` also means a future `iat` is not rejected; `iat` is deliberately not part of this design's claim rules.
